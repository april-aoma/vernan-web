/**
 * Vernan API ops ingest — latency / status for scores + auth workers.
 *   POST /api/events
 *
 * Storage: Analytics Engine (`OPS` → dataset `vernan_api_ops`).
 *
 * AE column layout:
 *   indexes[0] = event name (api_request)
 *   blobs[0]   = name
...
export interface Env {
  OPS: AnalyticsEngineDataset;
  RATE_LIMITER: DurableObjectNamespace;
}

const ALLOWED_NAMES = new Set(["api_request"]);
...
async function rateLimited(env: Env, ip: string): Promise<boolean> {
  const id = env.RATE_LIMITER.idFromName(`ops:${ip}`);
...
function parseEvent(raw: unknown): IncomingEvent | null {
  ...
  const method = sanitizeMethod(props.method);
  if (!route || !method) return null;
  return { name, props: { ...props, service, route, method } };
}

function writeEvent(env: Env, ev: IncomingEvent): void {
  const status = Math.floor(numProp(ev.props, "status", 0));
  const latency = Math.max(0, numProp(ev.props, "latency_ms", 0));
  ...
  env.OPS.writeDataPoint({
    ...
    blobs: [
      ...
      sanitizeMethod(ev.props.method),
      "",
    ],
    doubles: [status, latency, ok],
  });
}
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // Service-binding calls may not have CF-Connecting-IP; still rate-limit.
