...
/** Map auth error message → safe reason_code (no PII). */
...
/**
 * After a response is built, emit ops (+ security when applicable).
 * Call via `ctx.waitUntil(reportRequestTelemetry(...))`.
 */
export async function reportRequestTelemetry(
  ...
  meta: {
    ...
    status: number;
    latency_ms: number;
    /** Response JSON `error` field when present — used only for reason mapping. */
    errorMessage?: string;
  },
): Promise<void> {
...
/** Best-effort extract `{ error: string }` from a JSON Response clone. */
