...
- **Peer platforms + separation** (`EnemyPeerPlatforms`, `EnemyPeerSeparation`, `EnemyPeerTick`): enemies stand on each other's hull tops (moving decks), crawlers `ride_deck` on carriers, horizontal bump pushes + patrol flip, vertical stack snap; Possessed + Golden Roach excluded; **ice blocks are full solids** for walkers (sides + ceiling + deck, via `EnemyIceSolids` / Java `EnemyIceSolids`)
...
- **Bottom HUD** (Java `BottomHudLayout` / `drawBottomHud`): black 64px band; hearts 16px + half frames; coin/key + `formatMoneyHud`; combat stats (dmg×2 / squat / windup); passive strip newest-first; weapon+sub frames; **minimap at the right end** (web; Java keeps map left of touch chrome); money/key drain/gain anim
...
- **Pause**: Enter/Esc or shell left-shoulder **II** toggles `paused`; freezes sim; dim overlay + “PAUSE” + item grid menu (`PauseOverlay`); button highlights while paused
- **Display shell** (web-only, `DisplayShell.ts` / `gameDisplay.ts`): keeps the game at 512×320; aspect-fits into a play rect; landscape side gutters / portrait bottom band for controls; native orientation (no force-landscape CSS rotate)
- **Virtual controller** (web-only, `VirtualController.ts` + `CirclePad.ts`): always-on shell chrome — circle pad + clustered Z/X/C circles + L **II** pause / R Shift dodge; stick/face sides parameterized for a future flip; multitouch soft keys; fades with k-candy `TOUCH_CONTROLS`. HUD mini D-pad/JUMP/ATK chrome removed (vestigial).
...
- **Horizontal deck exemption**: leading-column wall scan; landing/resting decks defer to vertical (no `vx` kill on floor lips); `poseForFeetSupport` for jump-hull thin-deck probes; feet-near-deck dismissal requires **feet span ∩ tile column** (not Y-only / pose-bottom) so jump side-hits on ledge faces still block
